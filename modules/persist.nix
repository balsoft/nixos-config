...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    boot.initrd.systemd.services.create-nix-store = {
      enable = true;
      ...
      script = ''
        mkdir -p /sysroot/nix
        /bin/mount --bind /sysroot${cfg.persistRoot}/nix /sysroot/nix
        chmod 755 /sysroot
      '';
    };
      ...
      script = ''
        ${builtins.concatStringsSep "\n" (
          map (x: "rm ${lib.escapeShellArg x}") (
            absoluteEtcFiles cfg.cache.etcFiles ++ absoluteHomeFiles cfg.cache.homeFiles
          )
        )}
