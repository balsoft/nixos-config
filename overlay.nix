...
rec {
  ...
  pass-secret-service =
    (prev.pass-secret-service.override {
      python3 = final.python3 // {
        pkgs = final.python3Packages;
      };
    }).overrideAttrs
      (_: {
  ...
  mako = prev.mako.overrideAttrs (_: {
    postInstall = "sed 's|Exec=.*|Exec=/run/current-system/sw/bin/systemctl --user start mako|' -i $out/share/dbus-1/services/fr.emersion.mako.service";
  });
  ...
  okularMobile = final.kdePackages.okular.overrideAttrs (oa: {
    cmakeFlags = oa.cmakeFlags or [ ] ++ [ "-DOKULAR_UI=mobile" ];
  });
        ...
        in
        {
          ...
          patches = [
            (
              ...
              replaceVars
                (final.fetchpatch {
                  url = "https://raw.githubusercontent.com/nazarewk-iac/nix-configs/4eb0baf5e5b3692c07e626316257c115b7c79b3a/packages/overlays/pypass-mark-executables.patch";
