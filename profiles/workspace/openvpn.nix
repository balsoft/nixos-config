{ config, pkgs, lib, ... }: let password-store = config.secretsConfig.password-store; in {
  secrets.tawasal_eu1 = {
  ...
  systemd.services.openvpn-tawasal-eu1.wantedBy = lib.mkForce [];
  systemd.services.openvpn-tawasal-eu2.wantedBy = lib.mkForce [];
}
