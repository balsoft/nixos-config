# Generated by Powerlevel10k configuration wizard on 2022-08-24 at 17:24 +04.
...
() {
  ...
  function get_github_status() {
    ...
    if [[ -f "$cachefile" ]] && [[ "$(( $(stat -c %Y "$cachefile") + 60 ))" -gt "$(date +%s)" ]] && [[ -n "$(cat "$cachefile")" ]]; then
      ...
      local url="https://api.github.com/repos/$1/commits/$2/statuses"
      if [[ -n "$4" ]];
      then curl -s -H "Authorization: Bearer $4" "$url"
      else curl -s "$url"
      fi | tee /dev/stderr \
      | jq -r '. as $statuses | $statuses | map (.state) | unique | map(. as $state | {} | .[$state] = ($statuses | map(select(.state == $state)) | length)) | add as $result
        | if $result.pending > 0 then ["⌛",$result.pending|tostring] else [] end
        + if $result.success > 0 then ["✔",$result.success|tostring] else [] end
        + if $result.failure + $result.error > 0 then ["✘",$result.failure+$result.error|tostring] else [] end
        | join(" ")' \
      | tee "$cachefile" 
    fi
  }
